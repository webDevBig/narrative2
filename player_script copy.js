...
async function retrieve_page_info() {
	const response = await fetch('https://9r45litu0a.execute-api.us-east-1.amazonaws.com/default/narrativ-serve-article-audio', {
		method: 'POST',
		headers: {
		...
		body: JSON.stringify({
			...
			'article_host': window.location.host
		})
	})
...
async function initialize_narrativ() {
	...
	console.log("=== initializing narrativ ===")
	article_data = await retrieve_page_info();
